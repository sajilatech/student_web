			...
			<div class="page-head" id="showlinks">
				<ol class="breadcrumb">
                 ...
                 <li><a href="exams.php">Exams</a></li>
				  <li class="active">Exams Listing</li>
				</ol>
			</div>	
		<div class="cl-mcont">
				...
				<div class="col-md-12">
					<div class="block-flat">
						...
						<div class="content">
                            ...
                            <form role="form" method="post" action="exams.php">  
				 ...
				 foreach($student_arr as $row){
				  ...
				  <option value="<?php echo $row['student_ID'] ;?>" <?php echo (($row['student_ID']==$student_id)?'selected="selected"':'') ?> ><?php echo $row['name']; ?></option>
				     <?php } ?>
                                    ...
                                    <tr>
                                    <th style="width:5%;">Exam Model</th>
                                        <th style="width:30%;">
					...
					<div class="block-flat">
						<div class="header">							
                         ...
                         <div class="pull-left" style="padding-bottom:10px"><div class="dataTables_info" id="datatable_info">
                            <a href="exams.php?add=1"><div class="btn-group"><button class="btn btn-sm btn-success" type="button"><i class="fa fa-plus-square"></i></button><button class="btn btn-sm btn-default" type="button">Add New</button></div></a>                            
						...
						<div class="content">
							...
							<div class="table-responsive">
                            <form name="eventform" method="post" action="exams.php?doaction=deleterecord">
...
?>
										...
										<td><input name="list[]" id="list<?php echo $count_no; ?>" class="chkbx" value="<?php echo $row['appointment_ID']; ?>" type="checkbox"></td>
										 ...
										 <td><?php foreach($exam_model_list as $key => $value){ if ($key==$row['exam_model']){ echo $value; } } ?></td>
                                          <td><?php echo date('d-M-Y',$row['exam_date']); ?></td>
											<td><a data-placement="top" data-toggle="tooltip" data-original-title="Edit" class="label label-primary" href="exams.php?edit=1&select_id=<?php echo $row['exam_ID'];?>"><i class="fa fa-pencil"></i></a>
                                            <a data-placement="top" data-toggle="tooltip" data-original-title="Delete" class="label label-danger" href="exams.php?doaction=delete&select_id=<?php echo $row['exam_ID'];?>"><i class="fa fa-times"></i></a>
                                            <a  href="exams_view.php?select_id=<?php echo $row['exam_ID'];?>"><span class="badge">View</span></a>       
                                            </td>
	...
	for(i=0;i<a.length;i++){
		...
		else
		{
			if(vl == 3)
			{
				...
				{
				form_name.action = "exams.php?doaction=publish";
				form_name.submit();
				}
				else
				return false;
			...
			else if(vl == 4)
			{
				...
				{
				form_name.action = "exams.php?doaction=unpublish";
				form_name.submit();
				}
				else
				return false;
