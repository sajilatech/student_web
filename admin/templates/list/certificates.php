				...
				<div class="col-md-12">
					<div class="block-flat">
						...
						<div class="content">
                            ...
                            <form role="form" method="post" action="certificates.php">  
					...
					<div class="block-flat">
						<div class="header">							
                         ...
                         <div class="pull-left" style="padding-bottom:10px"><div class="dataTables_info" id="datatable_info">
                            <a href="certificates.php?add=1"><div class="btn-group"><button class="btn btn-sm btn-success" type="button"><i class="fa fa-plus-square"></i></button><button class="btn btn-sm btn-default" type="button">Add New</button></div></a>                            
						...
						<div class="content">
							...
							<div class="table-responsive">
                            <form name="eventform" method="post" action="certificates.php?doaction=deleterecord">
...
if($no_of_rows>0) 
{
	 ...
	 foreach($student_arr as $student_row){
		 $student_id=$student_row['student_ID'];
		 $certificates_arr=getResultArray("SELECT certificate_ID FROM ".MTABLE."student_certificates WHERE student_ID= $student_id "); 
		if(count($certificates_arr)>0){
$count_no++;
?>
                                    
										<tr class="gradeA <?php if($count_no%2!=0) { echo 'class="odd"'; } else { echo 'class="even"'; } ?>">
											<td><input name="list[]" id="list<?php echo $count_no; ?>" class="chkbx" value="<?php echo $student_row['student_ID']; ?>" type="checkbox"></td>
                                            <td><?php echo $count_no; ?></td>
                                            <td>
											<?php  echo $student_row['name']; ?></td>
											 <td>
											<?php  echo count($certificates_arr); ?></td>
											<td><a data-placement="top" data-toggle="tooltip" data-original-title="Edit" class="label label-primary" href="certificates.php?edit=1&select_id=<?php echo $student_row['student_ID'];?>"><i class="fa fa-pencil"></i></a>
                                            <a data-placement="top" data-toggle="tooltip" data-original-title="Delete" class="label label-danger" href="certificates.php?doaction=delete&select_id=<?php echo $student_row['student_ID'];?>"><i class="fa fa-times"></i></a>
                                             <a  href="certificates_view.php?select_id=<?php echo $student_row['student_ID'];?>"><span class="badge">View</span></a>
                                            </td>
	...
	for(i=0;i<a.length;i++){
		...
		else
		{
			if(vl == 3)
			{
				...
				{
				form_name.action = "certificates.php?doaction=publish";
				form_name.submit();
				}
				else
				return false;
			...
			else if(vl == 4)
			{
				...
				{
				form_name.action = "certificates.php?doaction=unpublish";
				form_name.submit();
				}
				else
				return false;
