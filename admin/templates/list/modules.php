			...
			<div class="page-head" id="showlinks">
				<ol class="breadcrumb">
                 ...
                 <li><a href="#">Course MGT</a></li>
				  <li class="active">Modules Listing</li>
				</ol>
			</div>	
		<div class="cl-mcont">
				...
				<div class="col-md-12">
					<div class="block-flat">
						...
						<div class="content">
                            ...
                            <form role="form" method="post" action="modules.php">  
							<table class="table no-border hover">
								<thead class="no-border">
									<tr>
										...
										<th style="width:30%;"><button class="btn btn-primary" type="submit">Search</button>
					...
					<div class="block-flat">
						<div class="header">							
                         ...
                         <div class="pull-left" style="padding-bottom:10px"><div class="dataTables_info" id="datatable_info">
                            <a href="modules.php?add=1"><div class="btn-group"><button class="btn btn-sm btn-success" type="button"><i class="fa fa-plus-square"></i></button><button class="btn btn-sm btn-default" type="button">Add New</button></div></a>                            
						...
						<div class="content">
							...
							<div class="table-responsive">
                            <form name="eventform" method="post" action="modules.php?doaction=deleterecord">
...
?>
										...
										<tr class="gradeA <?php if($count_no%2!=0) { echo 'class="odd"'; } else { echo 'class="even"'; } ?>">
											<td><input name="list[]" id="list<?php echo $count_no; ?>" class="chkbx" value="<?php echo $row['mdl_id']; ?>" type="checkbox"></td>
                                            <td><?php echo $count_no; ?></td>
                                            <td><?php echo $row['mdl_name']; ?></td>
                                            <td><?php echo $row['mdl_code']; ?></td>
											<!--<td><span class="statusdiv" id="statusdiv<?php echo $row['mdl_id']; ?>">
											<?php if($row['mdl_status'] != '2'){ ?> 
                                            <a data-placement="top" data-toggle="tooltip" data-original-title="Published" onclick="getStatus('ajax/changestatus.php?modules=1&id=<?php echo $row['mdl_id']; ?>&cstatus=<?php echo $row['mdl_status']; ?>','statusdiv<?php echo $row['mdl_id']; ?>')" class="label label-success"><i class="fa fa-check"></i></a>
											<?php } else { ?> 
                                            <a data-placement="top" data-toggle="tooltip" data-original-title="Unpublished" onclick="getStatus('ajax/changestatus.php?modules=1&id=<?php echo $row['mdl_id']; ?>&cstatus=<?php echo $row['mdl_status']; ?>','statusdiv<?php echo $row['mdl_id']; ?>')" class="label label-danger"><i class="fa fa-ban"></i></a>
											<?php } ?></span></td>-->
											<td><a data-placement="top" data-toggle="tooltip" data-original-title="Edit" class="label label-primary" href="modules.php?edit=1&select_id=<?php echo $row['mdl_id'];?>"><i class="fa fa-pencil"></i></a>
                                            <a data-placement="top" data-toggle="tooltip" data-original-title="Delete" class="label label-danger" href="modules.php?doaction=delete&select_id=<?php echo $row['mdl_id'];?>"><i class="fa fa-times"></i></a></td>
	...
	for(i=0;i<a.length;i++){
		...
		else
		{
			if(vl == 3)
			{
				...
				{
				form_name.action = "modules.php?doaction=publish";
				form_name.submit();
				}
				else
				return false;
			...
			else if(vl == 4)
			{
				...
				{
				form_name.action = "modules.php?doaction=unpublish";
				form_name.submit();
				}
				else
				return false;
