			...
			<div class="page-head" id="showlinks">
				<ol class="breadcrumb">
                 ...
                 <li><a href="#">Phone Calls</a></li>
				  <li class="active">Phone Calls Listing</li>
				</ol>
			</div>	
            <?php //$reminder_arr=getResultArray("SELECT * FROM ".MTABLE."call_reminders WHERE student_status = 1  ORDER BY student_ID asc "); ?>
		<div class="cl-mcont">
				...
				<div class="col-md-12">
					<div class="block-flat">
						...
						<div class="content">
                            ...
                            <form role="form" method="post" action="phone_chart.php">  
				 ...
				 foreach($student_arr as $row){
                  ...
                  <tr>
										<th style="width:5%;">Incoming/Outgoing</th>
										<th style="width:30%;"> <select name="phone_call_type" id="phone_call_type" class="form-control"  onchange="ajax_phone_number('../ajax/ajax_phone_number.php','divIncoming_outgoing')">
                  ...
                  <option value="<?php echo $key;?>" <?php if($phone_call_type == $key){?>selected="selected" <?php }?>><?php echo $value;?></option>
                   ...
                   <th style="width:5%;">Call Status</th>
                   <th style="width:30%;"><select name="phone_call_status" id="phone_call_status" class="form-control">
					...
					<div class="block-flat">
						<div class="header">							
                         ...
                         <div class="pull-left" style="padding-bottom:10px"><div class="dataTables_info" id="datatable_info">
                            <a href="phone_chart.php?add=1"><div class="btn-group"><button class="btn btn-sm btn-success" type="button"><i class="fa fa-plus-square"></i></button><button class="btn btn-sm btn-default" type="button">Add New</button></div></a>                            
						...
						<div class="content">
							...
							<div class="table-responsive">
                            <form name="eventform" method="post" action="phone_chart.php?doaction=deleterecord">
...
?>
										...
										<td><input name="list[]" id="list<?php echo $count_no; ?>" class="chkbx" value="<?php echo $row['phone_chart_ID']; ?>" type="checkbox"></td>
                                            <td><?php echo $count_no; ?></td>
                                             <td><?php echo $student_row['name']; ?></td>
                                          
										 <td><?php echo date('d-m-Y',$row['date']); ?></td>
                                           <td><?php  foreach($phone_call_type_list as $key=> $value){ 
										     if($key==$row['phone_call_type']){
												 echo $value;
											 }
										   }?></td>
											<td><a data-placement="top" data-toggle="tooltip" data-original-title="Edit" class="label label-primary" href="phone_chart.php?edit=1&select_id=<?php echo $row['phone_chart_ID'];?>"><i class="fa fa-pencil"></i></a>
                                            <a data-placement="top" data-toggle="tooltip" data-original-title="Delete" class="label label-danger" href="phone_chart.php?doaction=delete&select_id=<?php echo $row['phone_chart_ID'];?>"><i class="fa fa-times"></i></a>
                                               <a  href="phone_chart_view.php?select_id=<?php echo $row['phone_chart_ID'];?>"><span class="badge">View</span></a>
                                            </td>
	...
	for(i=0;i<a.length;i++){
		...
		else
		{
			if(vl == 3)
			{
				...
				{
				form_name.action = "phone_chart.php?doaction=publish";
				form_name.submit();
				}
				else
				return false;
			...
			else if(vl == 4)
			{
				...
				{
				form_name.action = "phone_chart.php?doaction=unpublish";
				form_name.submit();
				}
				else
				return false;
