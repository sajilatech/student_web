...
function delete_confirm(){
	mess1='Do you want to delete this item?';
	x = confirm(mess1); 
	if (x == true)
	{
	 return true;
			...
			<div class="page-head" id="showlinks">
				<ol class="breadcrumb">
                 ...
                 <li><a href="#">Students</a></li>
				  <li class="active">Students Listing</li>
				</ol>
			</div>	
		<div class="cl-mcont">
							...
							<table class="table no-border hover">
								<thead class="no-border">
									<tr>
										...
										<th style="width:30%;">
                                        <input type="text" id="search_txt" name="search_txt" class="form-control"></th>
                                       
										
									
                                    <th style="width:8%;">Category</th>
										<th style="width:30%;"><select name="student_category_id" id="student_category_id" class="form-control">
                                         <option value="">Select Category</option>
                     <?php 
   foreach($student_category_list as $key=>$value)
   {
	   echo '<option value="'.$key.'" '.(($key==$student_category_id)?'selected="selected"':'').'>'.$value.'</option>';
   }
   ?>
                  </select></th>
                  </tr>
					...
					<div class="block-flat">
						<div class="header">							
                         ...
                         <div class="pull-left" style="padding-bottom:10px"><div class="dataTables_info" id="datatable_info">
                            <a href="students.php?add=1"><div class="btn-group"><button class="btn btn-sm btn-success" type="button"><i class="fa fa-plus-square"></i></button><button class="btn btn-sm btn-default" type="button">Add New</button></div></a>                            
						...
						<div class="content">
							...
							<div class="table-responsive">
                            <form name="eventform" method="post" action="students.php?doaction=deleterecord">
...
?>
										...
										<tr class="gradeA <?php if($count_no%2!=0) { echo 'class="odd"'; } else { echo 'class="even"'; } ?>">
											<td><input name="list[]" id="list<?php echo $count_no; ?>" class="chkbx" value="<?php echo $row['crs_id']; ?>" type="checkbox"></td>
                                            <td><?php echo $count_no; ?></td>
                                            <td><?php echo $row['name']; ?></td>
                                            <td><?php echo $row['email']; ?></td>
                                          <td><?php echo $row['mst_id']; ?></td>
											<td><a data-placement="top" data-toggle="tooltip" data-original-title="Edit" class="label label-primary" href="students.php?edit=1&select_id=<?php echo $row['student_ID'];?>"><i class="fa fa-pencil"></i></a>
                                            <a data-placement="top" data-toggle="tooltip" data-original-title="Delete" class="label label-danger" onclick="return delete_confirm();" href="students.php?doaction=delete&select_id=<?php echo $row['student_ID'];?>"><i class="fa fa-times"></i></a> <a class="label label-warning" href="students_print.php?select_id=<?php echo $row['student_ID'];?>"><i class="fa fa-eye"></i></a>
                                            </td>
	...
	for(i=0;i<a.length;i++){
		...
		else
		{
			if(vl == 3)
			{
				...
				{
				form_name.action = "students.php?doaction=publish";
				form_name.submit();
				}
				else
				return false;
			...
			else if(vl == 4)
			{
				...
				{
				form_name.action = "students.php?doaction=unpublish";
				form_name.submit();
				}
				else
				return false;
