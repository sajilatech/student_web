		...
		if($camp_attended =='on'){
			...
			$c='0';
					...
					$questions_section_array=getResultArray("SELECT * FROM ".QUES_ANS_TBL." WHERE status = 1  AND section=2  AND question_type_ID='4' ORDER BY question_answer_ID asc ");
						...
						if(!empty($_POST['teacher_comment'])){
						 $sql_comment_data_array = array(
						 'teacher_ID' => $parish_teacher_id,
						 'student_ID'=> $insert_id,
						 'comments'=>$teacher_comment,
						 'added_date' => time());

						 $db->insert_from_array($sql_comment_data_array,TEACHER_COMMENT_TBL);	 
						}
						...
						$hobbies_count=count($hobbies_arr); 
							...
							$f_exam_pri_id = $db->insert_id(); 
								if($f_exam_pri_id !=''){ 
						 		...
						 		$db->UpdateQuery(EXAMTBL,$f_sql_percentage_array,$condition);
								}
							}
							
							$exam_model_array=getResultArray("SELECT * FROM ".MTABLE."exam_model_list WHERE status = 1  ORDER BY exam_model_list_ID asc ");
						  foreach($exam_model_array as $model){
							  print_r($_POST['marks_'.$model['exam_model_list_ID'].'_0']); exit;
						  }
							if(!empty($_POST['title']) || !empty($_POST['interview_comments']) || !empty($_POST['interviewed_by']) || !empty($_POST['psychological_results'])){
	...
	case 'update':
					...
					if($camp_attended =='on'){
						...
						foreach($questions_section_array as $row){
							...
							/* $db->query("DELETE FROM ".QUES_ANS_TBL." WHERE student_ID= ".$select_id." AND question_answer_ID'= ".$row['question+answer_ID']);*/
						...
						if(!empty($_POST['previous_school_name']) ){ 
						 ...
						 $db->query("DELETE FROM ".PREV_SCHOOL_HISTORY." WHERE student_ID= ".$select_id);
							...
							if($_POST['previous_syllabus'][$i] !="" && $_POST['previous_school_place'][$i] !="" ){
						...
						if($_POST['vp_name'] !=="" || $_POST['vp_name'] =='0'){
							$db->query("DELETE FROM ".VP_TBL." WHERE student_ID= ".$select_id);
						 $sql_vp_data_array = array(
						 'vp_name' => $vp_name,
						 'vp_phone' => $vp_phone,
						 'vp_address' => $vp_address,
						...
						if(!empty($_POST['teacher_comment'])){
							 $db->query("DELETE FROM ".TEACHER_COMMENT_TBL." WHERE student_ID= ".$select_id);
							$sql_teacher_comment_array = array(
							 ...
							 'added_date' => time());
						  	...
						  	$db->insert_from_array($sql_teacher_comment_array,TEACHER_COMMENT_TBL);
						}
					 ...
					 if(!empty($dislikes)){
						...
						$hobbies = $_POST['hobbies'];
							 ...
							 $sql_exam_result_array = array(
								...
								$condi="  student_ID = ".$select_id;				
						 		$ct=$db->get_count(EXAMTBL, $condi);
								if($ct!='0'){
									$sql_exam_array = array(
									'exam_type'=>'Academic', 
									'exam_date'=>$exam_date,
									...
									$total_marks=0; $total_outof=0;
									for($i=1;$i<=$no_subjects;$i++){ 
										if($_POST['subjects_'.$i] !=''){
											$total_marks= $total_marks + $_POST['marks_'.$i];
											 $total_outof= $total_outof + $_POST['outof_'.$i];
												
											$sql_exam_result_array = array(
												'exam_ID' =>$exam_id,
												 'subjects' => $_POST['subjects_'.$i],
												 'marks'=> $_POST['marks_'.$i],
												 'outof'=> $_POST['outof_'.$i]
												 ); 
												  $db->insert_from_array($sql_exam_result_array,EXAM_RESULT_TBL); 
										}
									}
									$total_percentage=($total_marks / $total_outof) * 100;
									$sql_percentage_array=array('percentage'=>$total_percentage);
									
									$condition=" student_ID= ".$select_id." AND exam_ID = ".$exam_id;
						 		$db->UpdateQuery(EXAMTBL,$sql_percentage_array,$condition);
								}
	...
	{ 
	  ...
	  list($family_name, $family_financial_status, $family_id, $reputation, $relation_with_parish_id)=$db->fetch_one_row("SELECT family_name, family_financial_status, family_id, reputation, relation_with_parish_id	FROM ".FAMILYTBL." WHERE student_ID='".$select_id."'"); 
		  ...
		  list($father_relation_id, $father_name, $father_nickname, $father_occupation, $relation_with_student, $father_mobile)=$db->fetch_one_row("SELECT student_relation_ID, relation_name, relation_nickname relation_occupation, relation_with_student, relation_phone, relation_phone	FROM ".RELATIONSTBL." WHERE student_ID='".$select_id."' AND relation_with_student='father'"); 
		   ...
		   list($creport_id, $student_id, $title, $interview_comments, $interview_results, $interviewed_by, $psychological_results, $description ) = $db->fetch_one_row("SELECT creport_ID, student_ID, title, interview_comments,  interview_results, interviewed_by, psychological_results, description FROM ".CONFIDENTIALTBL." WHERE student_ID='".$select_id."'");
